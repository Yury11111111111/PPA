...
class INumberInfo {
public:
    virtual bool is_number_simple(int number) = 0;
    virtual bool is_number_perfect(int number) = 0;

    virtual ~INumberInfo() = default;
};

class ICommonDivisors {
public:
    virtual int get_greatest_common_divisor(
        int number1,
        int number2
    ) = 0;

    virtual int get_least_common_multiple(
        int number1,
        int number2
    ) = 0;

    virtual ~ICommonDivisors() = default;
};

class Optimated_Divisors : public INumberInfo, public ICommonDivisors{
    public:
        bool is_number_simple(int number) override {
            check_number_in_divisors(number);
            return( data_divisors.at(number).size() == 2); 
        }
        bool is_number_perfect(int number) override {
            check_number_in_divisors(number);
            return (data_summs.at(number) == 2 * number);
        }
        int get_greatest_common_divisor(int number1, int number2) override {
            check_numbers_in_GCD(number1, number2);
            return data_GCD.at({number1, number2});
        }
        int get_least_common_multiple(int number1, int number2) override {
            check_numbers_in_GCD(number1, number2);
...
pair <bool, bool> get_number_info(int number, INumberInfo& number_info){
    return {
        number_info.is_number_simple(number),
        number_info.is_number_perfect(number)
    };
}

pair <int, int> get_GCD_and_LCM(int number1, int number2, ICommonDivisors& common_divisors){
    return {
        common_divisors.get_greatest_common_divisor(number1, number2),
        common_divisors.get_least_common_multiple(number1, number2)
    };
}

pair <int, int> reduce_fraction(int ret_up, int ret_down, ICommonDivisors& common_divisors){
    int reduction = common_divisors.get_greatest_common_divisor(abs(ret_up), ret_down);
    if (reduction != 1){
    ...
    string work (string input){
        if(count(input.begin(), input.end(), ' ')==1){
            try{
                ...
                pair <bool, bool> number1_info = get_number_info(number1, space);
                pair <bool, bool> number2_info = get_number_info(number2, space);
                pair <int, int> GCD_and_LCM = get_GCD_and_LCM(number1, number2, space);

                bool number1_simple = number1_info.first;
                bool number1_perfect = number1_info.second;
                bool number2_simple = number2_info.first;
                bool number2_perfect = number2_info.second;
                int GCD = GCD_and_LCM.first;
                int LCM = GCD_and_LCM.second;
            ...
            try{
                ...
                int ret_down = get_GCD_and_LCM(numbers.first.second, numbers.second.second, space).second;
                int ret_up = (numbers.first.first * ret_down /  numbers.first.second) + (numbers.second.first * ret_down /  numbers.second.second);

                pair <int, int> ret = reduce_fraction(ret_up, ret_down, space);
